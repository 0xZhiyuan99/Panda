...
def distinct_set():
    sqlite_connection = sqlite3.connect(setting.DB_PATH + "combined_new_signature_db.sqlite3")
    sqlite_cursor = sqlite_connection.cursor()
    sqlite_cursor.execute("select program, repeat_count from signatures where opcode_kinds=0")
...
def combine_database():
    final_db_path = setting.DB_PATH + "combined_segfault_signature_db.sqlite3"
    try:
...
def update_database():
    final_db_path = setting.DB_PATH + "combined_segfault_final_signature_db.sqlite3"
    try:
        os.unlink(final_db_path)
    except:
        pass
    ...
    final_sqlite_cursor.execute(signature_table_sql)
    count = 0
    for db_file in ["combined_segfault_signature_db.sqlite3", "combined_new_signature_db.sqlite3"]:
        current_db_path = setting.DB_PATH + db_file
        sqlite_connection = sqlite3.connect(current_db_path)
        sqlite_cursor = sqlite_connection.cursor()
        sqlite_cursor.execute("select * from signatures")
