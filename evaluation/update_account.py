...
signature_result_sql = "INSERT INTO signatures ( program, type, contain_app_call, include_app, repeat_count, opcodes, opcode_kinds, run_time, leaves_number, total_path, feasible_path, unchecked_transaction_fees, unchecked_rekey_to, unchecked_close_remainder_to, unchecked_asset_close_to ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
...
def distinct_set():
    ...
    sqlite_cursor.execute("select program, repeat_count from signatures where type=0")
...
def combine_database():
    final_db_path = setting.DB_PATH + "combined_update_signature_db.sqlite3"
    try:
...
def evaluate_smart_signatures(signature_list, identifier):
    ...
    for item in signature_list:
        ...
        sqlite_cursor.execute(signature_result_sql,(signature, 0, contain_app_call(file_content),include_app, repeat_count,
...
def update_database():
    final_db_path = setting.DB_PATH + "combined_account_signature_db.sqlite3"
    try:
        os.unlink(final_db_path)
    except:
        pass
    ...
    final_sqlite_cursor.execute(signature_table_sql)
    count = 0
    for db_file in ["combined_update_signature_db.sqlite3", "combined_segfault_final_signature_db.sqlite3"]:
        current_db_path = setting.DB_PATH + db_file
        sqlite_connection = sqlite3.connect(current_db_path)
        sqlite_cursor = sqlite_connection.cursor()
        sqlite_cursor.execute("select * from signatures")
