...
def distinct_set():
    sqlite_connection = sqlite3.connect(setting.DB_PATH + "combined_signature_db.sqlite3")
    sqlite_cursor = sqlite_connection.cursor()
    sqlite_cursor.execute("select program, repeat_count, include_app from signatures where include_app!=0")
...
def evaluate_smart_signatures(signature_list, identifier):
    ...
    for item in signature_list:
        ...
        app_id = int(item[2])
        try:
            if read_app_info(app_id) == get_app(app_id):
                continue
        except:
            continue
...
def update_database():
    final_db_path = setting.DB_PATH + "combined_new_signature_db.sqlite3"
    try:
        os.unlink(final_db_path)
    except:
        pass
    ...
    final_sqlite_cursor.execute(signature_table_sql)
    count = 0
    for db_file in ["combined_update_signature_db.sqlite3", "combined_signature_db.sqlite3"]:
        current_db_path = setting.DB_PATH + db_file
        sqlite_connection = sqlite3.connect(current_db_path)
        sqlite_cursor = sqlite_connection.cursor()
        sqlite_cursor.execute("select * from signatures")
